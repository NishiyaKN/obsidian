...
* **Formalismo:** Utiliza **Expressões Regulares** para definir os padrões dos tokens e **Autômatos Finitos** para reconhecê-los
* **Símbolos:** Os símbolos terminais de uma gramática correspondem aos tokens
* **Tratamento de Espaços:** O analisador léxico geralmente ignora espaços em branco, quebras de linha e comentários.
* **Tabela de Símbolos:** Os tokens identificados são organizados e entregues para a tabela de símbolos, que será utilizada pelo analisador sintático
...
* **Formalismo:** Utiliza um **Autômato de Pilha**, pois ele possui memória para lidar com a natureza aninhada das estruturas de programação
* **Árvore de Derivação:** Embora a análise resulte em uma estrutura de árvore, uma árvore explícita não é necessariamente construída na memória durante o processo
* **Interação:** A análise léxica e sintática podem ocorrer de maneira simultânea, embora o processo sempre precise iniciar com a análise léxica
...
| :--------------- | :---------------------------------------------------------------------- | :------------------- |
| **Gerador**      | Define as regras para gerar todas as palavras válidas de uma linguagem. | Gramáticas           |
| **Reconhecedor** | Verifica se uma determinada palavra pertence a uma linguagem.           | Autômatos            |
| **Denotacional** | Fornece uma notação simplificada para descrever uma linguagem.          | Expressões Regulares |
...
| :-------------------- | :--------------------------------------------------------------------------------------------------------------------- | :------------------- |
| **Regular**           | `A -> a` ou `A -> aB`. Lado esquerdo com uma variável, lado direito com no máximo uma variável.                        | Autômato Finito      |
| **Livre de Contexto** | `A -> x`. Lado esquerdo é uma única variável, e o lado direito é livre (qualquer combinação de terminais e variáveis). | Autômato de Pilha    |
...
* Detalhes sobre a prova e questões específicas mencionadas.
* Funcionamento de geradores de analisadores léxicos como o `lex`.
* Definições detalhadas de operações em linguagens (concatenação, fecho de Kleene, etc.).
* Uso e significado de metacaracteres específicos em expressões regulares.
* Gramáticas sensíveis ao contexto e a equivalência de autômatos com duas pilhas e Máquinas de Turing.
