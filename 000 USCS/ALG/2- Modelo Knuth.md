...
| **Índice de Array (ex: `a[i]`)** | $\sigma_{.}$ (cálculo de deslocamento/offset) | 1 operação (apenas o cálculo do índice, sem contar as recuperações). |
...
public static int Soma(int n) {
    int resultado = 0;                  // 2 operações (recupera 0, armazena)
    for (int i = 1; i <= n; ++i)        // Inicialização, Teste e Incremento
        resultado += i;                 // Corpo do loop (executado n vezes)
    return resultado;                   // 2 operações (lê e empilha)
}
```
