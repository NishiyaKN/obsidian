...
| :------------- | :----------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------------------------- |
| **Integração** | É um sistema completo em um único chip, com processador, memória e I/O integrados[cite: 1].      | Precisa de componentes externos como memória e periféricos de I/O para funcionar[cite: 1].     |
| **Aplicação**  | Projetado em hardware para executar uma tarefa específica e dedicada[cite: 1].                   | É de propósito geral, capaz de executar um sistema operacional complexo e múltiplos programas. |
| **Segurança**  | Processa apenas as informações para as quais foi projetado, não havendo risco de vírus[cite: 1]. | Suscetível a malwares e vírus, pois executa um software mais complexo.                         |

### 📡 Sensores: Coletando Dados do Ambiente
Sensores são dispositivos que detectam eventos ou mudanças no ambiente e enviam essa informação para o microcontrolador. Alguns tipos comuns incluem[cite: 1]:
* Temperatura
...
| :--- | :--- | :--- |
| **Efeito Hall** | Detecta a presença e a variação de intensidade de um campo magnético[cite: 1]. | Fornece um sinal de tensão proporcional à intensidade do campo, não apenas um "sim" ou "não"[cite: 1]. |
| **Reed Switch** | Funciona como uma chave simples que liga ou desliga na presença de um campo magnético[cite: 1]. | Um sinal digital (ligado/desligado)[cite: 1]. |

### ⚙️ Atuadores: Agindo no Mundo Físico
Enquanto os sensores coletam dados, os atuadores realizam ações.
* **Definição:** São dispositivos que transformam comandos e dados eletrônicos em ações no mundo físico[cite: 2]. Exemplos incluem motores, relés, LEDs e válvulas.
